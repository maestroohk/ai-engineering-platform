...
public class Infrastructure_Respects_ProcessBoundary
{
    private const string ActivationMilestone = "M4-D";
    private const string AdrReference = "ADR-016";

    [Fact(Skip = "Registered but disabled per ADR-016. Activates in M4-D when the first concrete provider implementation project (Providers.<X>) lands. The M4-A.1 slice ships the infrastructure seam; the boundary scan asserts the Infrastructure project is the only one that references System.Diagnostics.Process.")]
    public void Only_Infrastructure_MayReference_System_Diagnostics_Process()
    {
        var repoRoot = LocateRepoRoot();
        var infrastructureRoot = Path.Combine(repoRoot, "src", "AiEng.Platform.Infrastructure");
        var processReferencePattern = new Regex(
            @"using\s+System\.Diagnostics\b",
        ...
        foreach (var csproj in Directory.EnumerateFiles(repoRoot, "*.csproj", SearchOption.AllDirectories))
        {
            ...
            foreach (var path in Directory.EnumerateFiles(projectRoot, "*.cs", SearchOption.AllDirectories))
            {
                ...
                if (processReferencePattern.IsMatch(content))
                {
        ...
        Assert.True(
            failures.Count == 0,
            "Only the Infrastructure project (src/AiEng.Platform.Infrastructure) may reference System.Diagnostics.Process. " +
